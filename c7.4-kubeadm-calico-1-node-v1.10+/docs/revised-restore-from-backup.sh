...
mv "${MANIFEST_DIR}/kube-apiserver.yaml" "${MANIFEST_BACKUP_DIR}" || true
mv "${MANIFEST_DIR}/etcd.yaml" "${MANIFEST_BACKUP_DIR}" || true
...
mv /var/lib/etcd "${MNT_DISK}/var/etcd-corrupted"

# Replace the corrupted data dir with the restored data.
echo "Copying restored data to /var/lib/etcd (/var/etcd/data)"
